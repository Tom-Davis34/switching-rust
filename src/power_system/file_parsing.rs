...
pub fn parse_ps(path: &str) -> FileParseResults{
    ...
    let ps_nodes: Vec<PsNode> = bus_strs.iter().map(|s| PsNode::from_row(&s, &gens)).collect();
    ...
    let mut switches: Vec<Edge> = switch_strs.iter().map(|s| Edge::from_switch_row(&s, &ps_nodes_rc)).collect();
    ...
    let mut cicuits: Vec<Edge> = cicuits_strs.iter().map(|s| Edge::from_circuit_row(&s, &ps_nodes_rc)).collect();
...
fn parse_start_u(path: &str, name: &str) -> Vec<U> {
    ...
    switch_strs.iter()
    .map(|s| split_whitespace(s).get(2)
    .map(|s| s.parse::<i32>().unwrap() == 1).unwrap())
    .map(|b| { if b {
        U::Open
    }else {
        U::Closed
...
impl Edge {
    ...
    fn from_circuit_row(s: &str, nodes: &Vec<Rc<PsNode>>) -> Self {
        let cells = split_whitespace(s);
    ...
    fn from_switch_row(s: &str, nodes: &Vec<Rc<PsNode>>) -> Self{
        let cells = split_whitespace(s);
...
impl PsNode{
    fn from_row(s: &str, gens: &Vec<Gen>) -> Self {
        ...
        PsNode { num: num, load: load, gen: gen, n_type: nt}
    }
}